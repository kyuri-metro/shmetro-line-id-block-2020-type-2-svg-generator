...
const WHITE_TEXT_LINES = new Set([1, 4, 5, 6, 8, 11, 12, 14, 17]);
...
function getBadgePalette(lineNumber, foreground, background) {
...
function scaleLayout(layout, nextHeight) {
    ...
    return {
        ...
        textLayout: {
            ...
            letterSpacing: layout.textLayout.letterSpacing === undefined ? undefined : layout.textLayout.letterSpacing * scale,
...
function getBaseLayout(lineId, lineString) {
    ...
    const letterSpacing = lineString.length > 1 ? (scaledExpectedWidth - realWidth) / (lineString.length - 1) : 0;
    const totalLetterSpacing = letterSpacing * Math.max(0, lineString.length - 1);
    const centerX = lineId === 11 ? width * 0.44 : width / 2;
    const centerY = height / 2;
    const x = centerX / widthScale -
